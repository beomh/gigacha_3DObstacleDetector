...
void fusion_cloud_cb (const sensor_msgs::PointCloud2& ros_pc)
{
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessorI->Clustering(output_ptr, 0.4, 20, 1500);
...
{
  cout << "Fusion Obstacle Detection ON\n";
  // Initialize ROS
  ros::init (argc, argv, "FusionObstacleDetector");
  ros::NodeHandle nh;

  // // Create a ROS subscriber for the input point cloud
  ros::Subscriber sub = nh.subscribe("/bbox_filtered_pc", 1, fusion_cloud_cb);

  // Create a ROS publisher for the output point cloud
  pub1 = nh.advertise<sensor_msgs::PointCloud2> ("fusion_output", 1);
  //pub2 = nh.advertise<sensor_msgs::PointCloud2> ("clusters", 1);
  pub3 = nh.advertise<visualization_msgs::MarkerArray>("fusion_obstacles_markers", 10);
