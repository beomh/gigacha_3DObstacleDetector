...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes)
{
    ...
    // roof.setMin(Eigen::Vector4f(-3.0, -4.0, -0.4, 1.0));
    // roof.setMax(Eigen::Vector4f(3.0, 3.0, 3.0, 1.0));
    // roof.setInputCloud(cloud_filtered);
    // roof.filter(indices);

	// // all points in indices insert indices
